...
- Confirm the composer footer always shows the current model as `5.6 Sol` or the matching version/variant, the effective reasoning level without a `Default` marker, `Fast` only when that speed is effective, and `Read only`, `Workspace`, or `Full access` permission; its text and the Runtime trigger must match the Add/Send font size.
