...
- 作成日: 2026-07-14
- 状態: vNext Phase A 完了（読み取り専用会話画面）
...
- スレッド選択は Phase A の `openThread(threadId)` を呼び出し、会話パネルの生成処理を一覧側へ重複実装しない
...
完了条件: 公式 Codex 拡張に近い余白を持つスレッド一覧で、MVP の管理操作と Phase A の会話タブ起動を、キーボードとマウスの両方から利用できる。
...
- [x] Tree View のスレッドを選択すると、そのスレッド専用の会話タブが開く
- [x] 同じスレッドを再選択しても重複タブが増えない
...
| 一覧の Webview View 化でネイティブ操作を失う | キーボード、コンテキスト操作、アクセシビリティの回帰 | Extension Host に操作を集約し、フォーカス、ARIA、テーマ、メッセージ境界を Extension Host テストで確認 |
