...
- composer 下部へモデル、推論レベル、Fast 時だけの速度、Sandbox 権限を常時表示するようにした。モデルは `5.6 Sol` のように短縮し、既定値でも `Default` を付けず実効値だけを表示する。Runtime トリガーは短いラベルだけに戻し、トリガーとサマリーの文字サイズを Add／Send と揃えて、狭いサイドバーでも潰れにくくした。
