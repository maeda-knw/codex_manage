...
App Server の型は Codex CLI のバージョンに依存する。開発時に `codex app-server generate-ts` で生成した型をリポジトリへ固定し、更新用スクリプトも用意する。実行時にはレスポンスを無条件に信用せず、MVP で使う境界データを検証する。
...
| 設定 | 既定値 | 用途 |
| --- | --- | --- |
| `codexThreadManager.codexPath` | `codex` | PATH 外にある Codex CLI を指定 |
...
| リスク | 影響 | 対策 |
| --- | --- | --- |
| App Server が CLI 上で experimental 扱い | CLI 更新で型やメソッドが変わる | 型スナップショット、境界検証、互換性テスト、明示的な対応バージョン表示 |
