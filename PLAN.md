...
- 作成日: 2026-07-14
- 状態: vNext Phase A.6 完了（一覧 UI の整理とグループ折りたたみ）
...
- 48px のカード、管理メニュー、矢印／Home／End 移動、フォーカス表示、ARIA ラベルを追加した。
...
完了条件: 一覧内に重複したタイトルや操作がなく、各グループを折りたたんだ状態が画面遷移とウィンドウ再読み込みをまたいで維持される。
...
次の開発フェーズは vNext Phase B とし、サイドバー会話画面へ送信、ストリーミング、Stop を段階的に追加する。
